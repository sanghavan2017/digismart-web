...
- Form thu lead (`app/api/lead/route.ts`, `LeadForm.tsx`, `LeadFormButton.tsx`)
...
- System prompt chatbot (`app/api/chat/route.ts`) tự build từ `data/products.ts` — luôn đồng bộ khi data đổi, không cần sửa code riêng.

## ⚠️ Đang chặn / cần bạn xử lý (xem Task List trong Claude Code để theo dõi)
1. **Vercel production đang deploy nhánh `master` (data cũ AoSmith)**, không phải `cleansui-wip`. Domain thật `digismartvn.com` (lưu ý: ĐUÔI .com, không phải .vn như ghi nhầm trước đây) đã kết nối DNS đúng và đang live, nhưng vẫn hiện bản cũ vì branch sai. Cần merge `cleansui-wip` → `master`, hoặc đổi Production Branch trong Vercel Settings → Git.
2. **Data Điều hòa vẫn 100% giả** (Mitsubishi/Daikin placeholder, giá bịa) — cần SKU thật (brand đang bán, ví dụ Unimax) để làm tương tự Cleansui/Kitz.
3. **Biến môi trường (.env.local) chưa cấu hình trên Vercel (production)** — `ANTHROPIC_API_KEY` và `RESEND_API_KEY` đã điền đúng ở local (`.env.local`, không commit lên git) và đã test chatbot + form lead hoạt động tốt ở local. Nhưng Vercel project cần cấu hình lại 2 biến này riêng trong Settings → Environment Variables, nếu chưa có thì chatbot/form lead trên web thật sẽ không hoạt động.
4. **Resend đang ở chế độ test** (chưa verify domain riêng) — chỉ gửi email lead tới đúng 1 địa chỉ đã đăng ký tài khoản Resend (`digismart606@gmail.com`). Muốn gửi thêm tới `sang.havan2017@gmail.com` hoặc địa chỉ khác, cần verify 1 domain tại resend.com/domains.

## Đã xử lý gần đây
- ✅ Domain xác nhận: `digismartvn.com` đã kết nối Vercel, DNS đúng, site đang live (chỉ sai branch nội dung — xem mục #1 trên).
- ✅ 2 claim sai trên `/ve-chung-toi` đã sửa: bỏ "Logitech & Anker" (không đúng), bỏ "10.000 khách hàng" (không có số liệu thật) — thay bằng mốc "2024: mở rộng sang Điều hòa & Máy lọc nước, trở thành đại lý Cleansui & Kitz".
- ✅ API key Anthropic + Resend đã điền vào `.env.local` (local), test thành công: chatbot trả lời đúng theo data thật + đúng giọng văn thiết kế; form lead gửi email thành công tới `digismart606@gmail.com`.
