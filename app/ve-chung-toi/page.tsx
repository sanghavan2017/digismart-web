    ...
    <>
      ...
      <section style={{ background: "linear-gradient(135deg, #042C53 0%, #185FA5 70%, #378ADD 100%)", padding: "4rem 0" }}>
        <div className="container" style={{ textAlign: "center", color: "#fff" }}>
          ...
          <p style={{ fontFamily: "Calibri, sans-serif", fontSize: "1.05rem", opacity: 0.85, maxWidth: 560, margin: "0 auto", lineHeight: 1.7 }}>
      ...
      <section style={{ background: "#fff", padding: "4rem 0" }}>
        <div className="container">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "3rem", alignItems: "center" }}>
            <div>
              <h2 style={{ fontFamily: "'Trebuchet MS', sans-serif", fontSize: "1.6rem", color: "var(--brand)", marginBottom: "1rem" }}>
            ...
            <div style={{ background: "var(--brand-light)", borderRadius: 12, padding: "2.5rem", border: "1px solid var(--border)" }}>
              <div style={{ fontFamily: "'Trebuchet MS', sans-serif", fontSize: "1.1rem", color: "var(--brand)", fontWeight: 700, marginBottom: "1.5rem" }}>
                Con số nổi bật
              </div>
              {[
                ...
                <div key={s.label} style={{ display: "flex", justifyContent: "space-between", padding: "0.6rem 0", borderBottom: "1px solid var(--border)" }}>
                  <span style={{ fontFamily: "Calibri, sans-serif", color: "var(--muted)", fontSize: "0.9rem" }}>{s.label}</span>
                  <span style={{ fontFamily: "'Trebuchet MS', sans-serif", fontWeight: 700, color: "#F07B20", fontSize: "1rem" }}>{s.num}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ background: "var(--bg)", padding: "4rem 0" }}>
        <div className="container">
          <h2 style={{ fontFamily: "'Trebuchet MS', sans-serif", fontSize: "1.5rem", color: "var(--brand)", marginBottom: "2rem", textAlign: "center" }}>
            Giá trị cốt lõi
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1.5rem" }}>
            {values.map(v => (
              <div key={v.title} style={{ background: "#fff", borderRadius: 10, padding: "1.75rem", border: "1px solid var(--border)" }}>
                ...
                <div style={{ fontFamily: "'Trebuchet MS', sans-serif", fontWeight: 700, fontSize: "0.95rem", color: "var(--brand)", marginBottom: "0.5rem" }}>{v.title}</div>
                <div style={{ fontFamily: "Calibri, sans-serif", fontSize: "0.88rem", color: "var(--muted)", lineHeight: 1.65 }}>{v.desc}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            {milestones.map((m, i) => (
              <div key={m.year} style={{ display: "flex", gap: "1.5rem", marginBottom: i < milestones.length - 1 ? "1.5rem" : 0 }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                  <div style={{ width: 44, height: 44, borderRadius: "50%", background: "var(--brand)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Trebuchet MS', sans-serif", fontWeight: 700, fontSize: "0.8rem" }}>
                    {m.year}
                  </div>
                ...
                <div style={{ paddingTop: 10 }}>
                  <p style={{ fontFamily: "Calibri, sans-serif", fontSize: "0.95rem", color: "var(--text)", lineHeight: 1.6 }}>{m.event}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: "var(--brand)", padding: "3rem 0", textAlign: "center" }}>
        <div className="container">
          ...
          <div style={{ display: "flex", gap: "1rem", justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <a href="/lien-he" style={{ background: "transparent", color: "#fff", border: "1.5px solid rgba(255,255,255,0.5)", padding: "12px 28px", borderRadius: 6, fontWeight: 500, textDecoration: "none", fontFamily: "Trebuchet MS, sans-serif" }}>
              Gửi tin nhắn
            </a>
          </div>
        </div>
